...
import train_code.train_crnn.crnn as crnn
import config
...
config.saved_model_prefix = 'CRNN-5'
#config.train_infofile = ['path_to_train_infofile1.txt','path_to_train_infofile2.txt']
config.train_infofile = 'D:/OCR/train_code/train_crnn/train_data/train.txt'
config.val_infofile = 'D:/OCR/train_code/train_crnn/val_data/val.txt'
...
if config.pretrained_model!='' and os.path.exists(config.pretrained_model):
    ...
    temp=torch.load(config.pretrained_model,map_location=device)
    crnn.load_state_dict(temp)
else:
    print('the pretrained model download failed!')
