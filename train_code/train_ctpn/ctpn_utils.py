...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    # calculate iou
    overlaps = cal_overlaps(base_anchor, gtboxes)          #比较生成的候选框和实际有文字的候选框的重合度，选取最佳的候选框

    # init labels -1 don't care  0 is negative  1 is positive  如果不是文字框，则标识成0；如果是含文字的候选框，则标记成1；如果候选框越界，则标记成-1.
    ...
    # IOU > IOU_POSITIVE
    labels[anchor_max_overlaps > IOU_POSITIVE] = 1     #阈值为0.7，大于0.7则标记成1，是含文字的候选框。
    # IOU <IOU_NEGATIVE
