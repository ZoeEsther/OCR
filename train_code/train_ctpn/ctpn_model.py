...
class RPN_REGR_Loss(nn.Module):          #提取候选框的回归损失  （判断检测框的大小是否合适）
...
class RPN_CLS_Loss(nn.Module):  #提取候选框的分类损失 （判断是否为文字区域）
...
class CTPN_Model(nn.Module):
    def __init__(self):
        super().__init__()
        base_model = models.vgg16(pretrained=False)            ##加载 vgg16 模型，只导入模型，不导入参数
        layers = list(base_model.features)[:-1]
        self.base_layers = nn.Sequential(*layers)                 # block5_conv3 output
        self.rpn = basic_conv(512, 512, 3, 1, 1, bn=False)
        self.brnn = nn.GRU(512,128, bidirectional=True, batch_first=True)       #加入RNN，沿 w 的方向
        self.lstm_fc = basic_conv(256, 512, 1, 1, relu=True, bn=False)            #因为是双向LSTM，所以特征从128变成256
        self.rpn_class = basic_conv(512, 10 * 2, 1, 1, relu=False, bn=False)
        self.rpn_regress = basic_conv(512, 10 * 2, 1, 1, relu=False, bn=False)

    def forward(self, x):
        #print ('1:',x.size())
        x = self.base_layers(x)       #  前向传播
        #print ('2:',x.size())
        # rpn
        x = self.rpn(x)    #[b, c, h, w]
        #print ('3:',x.size())
        x1 = x.permute(0,2,3,1).contiguous()  # channels last   [b, h, w, c]   为了满足RNN网络沿W方向的条件
        #print ('4:',x1.size())
        b = x1.size()  # b, h, w, c
