...
weights = os.path.join(config.checkpoints_dir,'CTPN3.pth')# CTPN1.pth ctpn_ICDAR_ep28_0.3658_0.1076_0.4734.pth'v3_ctpn_ep28_0.3658_0.1076_0.4734.pth'
model = CTPN_Model()
...
def get_det_boxes(image,display = True, expand = True):
    ...
    with torch.no_grad():
        ...
        keep = nms(nmsbox, 0.3)    # 非极大值抑制 与之相关的IOU   ，nms 的阈值刚开始是0.3
        # print(keep)
        ...
        if display:
            ...
            for box in select_anchor:
                pt1 = (box[0], box[1])
                pt2 = (box[2], box[3])
                blank = cv2.rectangle(blank, pt1, pt2, (0,0,0), -1)  # b.g.r  最终确定的box 内部画蓝色，改为白色
            image_c = image_c+blank
            image_c[image_c>255] = 255
            for i in text:
                #s = str(round(i[-1] * 100, 2)) + '%'    ############# 最终输出图片上的百分比数  ###########
                i = [int(j) for j in i]
...
if __name__ == '__main__':
    ...
    img_path = 'D:/OCR/detect/data/006.jpg'
